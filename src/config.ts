...
export interface Settings {
  minIntervalMs: { global: number; search: number; comments: number; profile: number; invite: number; connections: number; message: number };
  jitterMs: number;
  // Jitter spécifique par type d'appel (surcharge jitterMs). Sert surtout aux invitations
  // et messages, où l'on veut un espacement humain (ex. invite : minInterval 60s + jitter 60s = 60-120s).
  jitterMsByKind?: Partial<Record<'search' | 'comments' | 'profile' | 'invite' | 'connections' | 'message', number>>;
  dailyCaps: { voyager: number; profile: number; searchPeople: number; invite: number; connections: number; message: number };
  retry: { maxRetries: number; baseDelayMs: number; maxDelayMs: number; backoffFactor: number };
  defaultServerCooldownMs: number;
}

const DEFAULT_SETTINGS: Settings = {
  minIntervalMs: { global: 4000, search: 9000, comments: 3000, profile: 14000, invite: 60000, connections: 9000, message: 45000 },
  jitterMs: 5000,
  jitterMsByKind: { invite: 60000, message: 45000 },
  dailyCaps: { voyager: 300, profile: 50, searchPeople: 100, invite: 20, connections: 100, message: 40 },
